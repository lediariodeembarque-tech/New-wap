...
export default function App() {
  ...
  const searchDeparture = () => {
    ...
    notify('Abrindo partidas do GRU');
  };
  const generatePdf = () => {
    const doc = new jsPDF();
    ...
    doc.setTextColor(240, 245, 252); doc.setFontSize(10); doc.text(`Data: ${new Date(`${date}T12:00:00`).toLocaleDateString('pt-BR')} | ${user}`, 14, 28);
    let y = 43;
    flights.forEach((flight, index) => {
      if (y > 265) { doc.addPage(); y = 18; }
      doc.setTextColor(245, 198, 72); doc.setFontSize(12); doc.text(`VOO ${flight.number || index + 1} - ${flight.destination || 'Sem destino'}`, 14, y);
  ...
  const menuOpenStyle = { position: 'absolute', right: 0, top: 44, zIndex: 5, minWidth: 150, padding: 8, borderRadius: 12, background: '#12273d', border: '1px solid rgba(255,255,255,.16)', boxShadow: '0 12px 30px #0008' };
...
function Field({ label, value, onChange }) {
  ...
  return <label className="field-block"><span style={{ color: accent }}>{label}</span><input value={value} onChange={(event) => onChange(event.target.value)} /></label>;
}
